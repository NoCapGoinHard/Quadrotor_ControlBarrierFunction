...
v_cbf = obs_4dot -(2*z_dot+mu2*z_2dot+mu3*z_3dot)/mu4 -(mu3*z_dot'*z_2dot+mu4*z_dot'*z_3dot)*(z+mu2*z_dot+mu3*z_2dot+mu4*z_3dot)/(mu4*h) +(eye(3)-(z*z')/(z'*z))*v_star(1:3);
...
a=min(exp(-(h-delta1)/0.01),1); % a=1 if h<=delta1, a=0 if h>delta1

% "b" is equivalent to check "h_dot_star<=0"
b=min(exp(-h_dot_star/0.001),1); % b=1 if h_dot_star<=0, b=0 if h_dot_star>0
...
disp(['t = ', num2str(t), ', h = ', num2str(h), ', h_dot_star = ', num2str(h_dot_star), ', a = ', num2str(a),  ', b = ', num2str(b),  ', a*b = ', num2str(coeff), '.']);
end
